...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float f, boolean flag, int i, int j) {
        ...
        if (mc.gameSettings.showDebugInfo) {
            ...
            // WETHACK - Calling the render screen event
            // We only want to do this when we aren't rendering the debug information so that it doesn't get in the way
            final RenderScreenEvent renderScreenEvent = new RenderScreenEvent(scaledresolution);
            WetHack.INSTANCE.getEventBus().post(renderScreenEvent);
        }
